...
const MeetingInfoPage = () => {
  ...
  const dataSource = data ? [
    ...
    { key: '7', label: '参加者', value: <ul>{data.participants.map((participant: string, index: number) => <li key={index}>{participant}</li>)}</ul> },
    { key: '8', label: 'アジェンダ', value: <ul>{data.agenda.map((item: any, index: number) => <li key={index}>{item.topic} - {item.duration}分</li>)}</ul> },
  ] : [];
